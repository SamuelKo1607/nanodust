...
beta_bound_isd_model <- function(cmd = c("graph", "Q", "mu", "initial", 
  ...
  prior.e_a_r <- function(e_a_r=feed_x){
    return(dnorm(e_a_r,    mean  = -1.1, sd   = 0.1, log=TRUE))
  }
  prior.e_b_r <- function(e_b_r=feed_x){
    return(dnorm(e_b_r,    mean  = -1.65, sd   = 0.5, log=TRUE))
  }
  
  rate <- function(v_sc_r, v_sc_t, r_sc, v_sc_x, v_sc_y, v_sc_z,   #covariates
                   l_a, l_isd, l_b, v_b_r, e_v, e_a_r, e_b_r,      #hyperparameters
                   S=8,                                    #sc parameters
                   v_isd=26, phi_isd=259, th_isd=8,        #isd parameters
                   v_b_a=9, v_earth_a=0){                  #beta met. parameters
    ...
    v_factor = (   ( ( v_isd_x - v_sc_x )^2 
                  +  ( v_isd_y - v_sc_y )^2 
                  +  ( v_isd_z - v_sc_z )^2 )^0.5 
                ) / (v_isd)
    L_isd = l_isd * (v_factor)^1 * 1^1
    ...
    hourly_rate = 3600 * S * ( L_a + L_isd + L_b )
    return(hourly_rate)
  }
  ...
  initial <- function(){
    ...
    return(c(log(optimize(prior.l_a,   interval = c(0, 1e-2), maximum = TRUE, tol=1e-9)$maximum),
             ...
             optimize(prior.e_v,       interval = c(-10, 10), maximum = TRUE, tol=1e-6)$maximum,
             optimize(prior.e_a_r,     interval = c(-10, 10), maximum = TRUE, tol=1e-6)$maximum,
             optimize(prior.e_b_r,     interval = c(-10, 10), maximum = TRUE, tol=1e-6)$maximum
            )
          )
  }
...
rgen = inla.rgeneric.define(model = beta_bound_isd_model, 
                            vr = mydata$vr, 
                            vt = mydata$vt, 
                            r  = mydata$r,
                            vx = mydata$vx,
...
prior.l_a  <- function(x){ 
  return((beta_bound_isd_model(cmd="prior.l_a", feed_x=x))) }
prior.l_isd <- function(x){
  return((beta_bound_isd_model(cmd="prior.l_isd",feed_x=x))) }
prior.l_b   <- function(x){
  return((beta_bound_isd_model(cmd="prior.l_b",  feed_x=x))) }
prior.v_b_r <- function(x){
  return((beta_bound_isd_model(cmd="prior.v_b_r",feed_x=x))) }
prior.e_v   <- function(x){
  return((beta_bound_isd_model(cmd="prior.e_v",  feed_x=x))) }
prior.e_a_r   <- function(x){
  return((beta_bound_isd_model(cmd="prior.e_a_r",  feed_x=x))) }
prior.e_b_r   <- function(x){
  return((beta_bound_isd_model(cmd="prior.e_b_r",  feed_x=x))) }
#priors evaluated
#l_a
...
hexbinplot(sample_l_a~sample_l_isd, 
           data=data.frame(sample_l_a,sample_l_isd), 
           colramp=colorRampPalette(c("grey", "blue")),
           main="joint histogram l_a, l_isd" ,  
           xlab="l_isd", 
           ylab="l_a" ,
           panel=function(x, y, ...)
           {
             panel.hexbinplot(x, y, ...)
             panel.abline(v=c(mean(sample_l_isd)), h=c(mean(sample_l_a)), col="black", lwd=2, lty=3)
           }
)
...
model_definition <- deparse(beta_bound_isd_model)
...
save(sample_l_a,       #sampled posterior
     ...
     file = paste("998_generated\\inla\\sample_bound_",formatted_time,".RData",
                  sep = ""))
